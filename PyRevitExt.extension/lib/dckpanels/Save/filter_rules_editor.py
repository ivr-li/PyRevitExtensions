...
class ParameterInfo(object):
    ...
    def __init__(self, name, param_id, storage_type="string", spec_type_id=None):
        self.name = name
...
def _to_internal(value_str, doc, param_info):
    """Convert display-unit value string to Revit internal units."""
    num = _parse_number(value_str)
    if not param_info or not param_info.spec_type_id:
        return num
    unit_id = _get_display_unit(doc, param_info.spec_type_id)
    if not unit_id:
        return num
    try:
...
def _from_internal(internal_val, doc, spec_type_id):
    """Convert Revit internal units to display-unit string."""
    if not spec_type_id:
        return str(internal_val)
    unit_id = _get_display_unit(doc, spec_type_id)
    if not unit_id:
        return str(internal_val)
    try:
        converted = UnitUtils.ConvertFromInternalUnits(internal_val, unit_id)
...
def _get_bip_storage_info(doc, param_id, category_ids=None):
    ...
    st_str, spec = _resolve_bip_via_forge(doc, bip, category_ids)
    _bip_cache[int_val] = (st_str, spec)
    return (st_str, spec)


def _resolve_bip_via_forge(doc, bip, category_ids=None):
    """Resolve storage type and spec via ParameterUtils (Revit 2022+)."""
    ...
    try:
        ...
        if st_str != "number":
            return (st_str, None)
        spec = _try_forge_id_as_spec(forge_id)
        if not spec:
            spec = _find_bip_spec_on_element(doc, bip, category_ids)
        return (st_str, spec)
    except (AttributeError, Exception):
        pass
    return ("string", None)
...
def _try_forge_id_as_spec(forge_id):
    """Check if parameter ForgeTypeId is also a valid measurable spec."""
    if not forge_id:
        return None
    try:
...
def _find_bip_spec_on_element(doc, bip, category_ids):
    """Fallback: get spec from parameter Definition on a real element."""
    if not category_ids:
        return None
    for cat_id in category_ids:
        try:
            bic = BuiltInCategory(cat_id.IntegerValue)
        except Exception:
            continue
        elem = _first_element_in_category(doc, bic)
        if not elem:
            continue
        param = elem.get_Parameter(bip)
        if not param or not param.Definition:
            continue
        spec = _get_definition_data_type(param.Definition)
        if not spec:
            continue
        if _try_forge_id_as_spec(spec):
            return spec
    return None


def _first_element_in_category(doc, bic):
    """Get first available element (instance or type) in a category."""
    elem = (
...
def _get_param_storage_info(doc, param_id, category_ids):
    """Return (storage_type_str, spec_type_id) via Revit API."""
    try:
        if param_id.IntegerValue < 0:
            return _get_bip_storage_info(doc, param_id, category_ids)
...
def _get_rule_value(rule, doc=None, spec_type_id=None):
    ...
    if isinstance(inner, FilterDoubleRule):
        return _from_internal(inner.RuleValue, doc, spec_type_id)
...
def parse_element_filter(doc, pfe):
    ...
    Returns:
        (str, list) or None if filter is not supported.
    """
        ...
        results.append(
            {
                ...
                "value": _get_rule_value(rule, doc, spec),
...
def _build_single_rule(rule_item, doc=None):
    ...
    if key == "num_not_equals":
        num_val = _to_internal(value, doc, rule_item.SelectedParameter)
...
def get_filterable_parameters(doc, category_ids):
    ...
    try:
        ...
        for pid in param_ids:
            ...
            storage, spec = _get_param_storage_info(doc, pid, category_ids)
            params.append(ParameterInfo(name, pid, storage, spec))
    except Exception:
...
def _find_param(available_params, param_id, fallback_name, spec_type_id=None):
    """Find ParameterInfo by id, or create a fallback."""
    for p in available_params:
        if p.param_id == param_id:
            return p
    return ParameterInfo(fallback_name, param_id, spec_type_id=spec_type_id)
