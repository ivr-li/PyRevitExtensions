...
def pick_color(current_rgb):
    """Open Revit native color picker dialog."""
    dlg = ColorSelectionDialog()
    if current_rgb:
        dlg.OriginalColor = Color(current_rgb[0], current_rgb[1], current_rgb[2])
...
def get_indicator_colors(view, filter_id):
    """Return (fill_brush, border_brush, tooltip) for override indicator."""
    ovr = view.GetFilterOverrides(filter_id)
    accent = WinMedia.SolidColorBrush(WinMedia.Color.FromRgb(0x5F, 0x9E, 0xA0))

    proj_color = _get_first_override_color(ovr, is_projection=True)
    cut_color = _get_first_override_color(ovr, is_projection=False)

    has_proj = proj_color is not None
    has_cut = cut_color is not None

    if has_proj and has_cut:
        tip = "Заливка: проекция, рамка: разрез"
        return (_rgb_to_brush(proj_color), _rgb_to_brush(cut_color), tip)
    if has_proj:
        return (_rgb_to_brush(proj_color), None, "Проекция")
    if has_cut:
        return (_rgb_to_brush(cut_color), None, "Разрез")
    return (accent, None, "Графика изменена")
